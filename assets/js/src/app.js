...
var BREW_BASE = 'https://api.brewerydb.com/v2/';
var BREW_KEY = 'd64d6b3db7955a685cf1b5486a326fd4';
...
function AppViewModel(googleMap) {
  ...
  appView.buildInfoBox = function (item) {
    ...
    var website = '';
    var established = '';
    var image = 'assets/images/beer.jpeg';
  ...
  appView.filterLocations = function (type) {
    var locationsToDisplay = appView.locations.filter(function (item) {
      if (type === 'all') {
        return item;
  ...
  appView.stopMarkers = function () {
    for (var i = 0; i < appView.displayedLocations().length; i++) {
  ...
  appView.getData = function () {
    ...
    var promise = $.getJSON('http://localhost/map/assets/js/testData.json');
  ...
  appView.plotMapMarkers = function () {
    var map = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : appView.map;

    for (var i = 0; i < appView.mapMarkers.length; i++) {
